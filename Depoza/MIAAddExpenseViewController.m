...
@interface MIAAddExpenseViewController () <UITextFieldDelegate, UITableViewDataSource, UITableViewDelegate>
...
@implementation MIAAddExpenseViewController {
...
- (void)doneBarButtonPressed:(UIBarButtonItem *)doneBarButton {
    ...
    if (_expenseFromTextField.floatValue > 0.0f && _isChosenCategory) {
        MIAExpense *expense = [MIAExpense expenseWithAmount:_expenseFromTextField categoryName:_categories[_selectedRow.row] description:_descriptionTextField.text];
...
- (void)addExpenseToCategoryData:(MIAExpense *)expense {
